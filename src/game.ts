...
// Consider that these initial definitions be read only
export type SpaceStatus = "correct" | "present" | "absent";

// Fill in the instructed properties
export type Space = {
...
export function renderSpace(space: Space) {
    switch (space.status) {
        case 'correct':
...
export function convertGuessedWordToRow(guessedWord: string, word: string, row: Row): Row {
...
export function endGame(score: Score) {
    // TODO: Write the score to a file
    writeFile('scores/.gitkeep', score); { // need to write in date in this function and link the actual score so it's not coming back as 'score = [object Object]'
...
export function guessWord(words: string[], promptCallback: (message: string) => string
): string | Error {
...
export function gameLoop(board: Board, words: string[], word: string, promptCallback: (message: string) => string): Score {
    let guessCount = 0
